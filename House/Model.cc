  ...
  _texcoords(tex_coords)
{
	assert(vertices != nullptr &&
			"Параметр 'vertices' конструктора Model "
			"не может иметь значение nullptr!");
	assert(faces != nullptr &&
			"Параметр 'faces' конструктора Model "
			"не может иметь значение nullptr!");
	assert(normals != nullptr &&
			"Параметр 'normals' конструктора Model "
			"не может иметь значение nullptr!");
	assert(tex_coords != nullptr &&
			"Параметр 'tex_coords' конструктора Model "
			"не может иметь значение nullptr!");
}
...
Model::Model(const char *filename)
{
	...
	//Читаем описание вершин
	read_model_line(file, buffer);
	std::stringstream (buffer) >> vertux_count;
	vertices.resize(vertux_count);
	for (unsigned i = 0; i < vertux_count; ++i) {
		read_model_line(file, buffer);
	...
	//Читаем описание нормалей
	normals.resize(face_count);
	for (unsigned face = 0; face < face_count; ++face) {
		read_model_line(file, buffer);
		std::stringstream(buffer) >>
				normals[face].x >> normals[face].y >> normals[face].z;
	}
